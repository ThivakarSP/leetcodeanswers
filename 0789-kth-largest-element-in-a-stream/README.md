...
**Runtime**: `21 ms` (Beats 93.65%)

**Memory**: `52.1 MB` (Beats 74.03%)
...
**Notes**:
Uses a min‑heap to keep the k largest elements; each add operation inserts the new value and removes the smallest if the heap exceeds size k, ensuring the heap size never exceeds k.
