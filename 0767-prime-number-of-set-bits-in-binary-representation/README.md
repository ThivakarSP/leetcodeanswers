...
**Notes**:
The solution iterates over each integer in the range, uses Java's bitCount to count set bits, and checks primality of that count with a simple loop. Both operations are constant‑time for 32‑bit integers, yielding linear time and constant space.
