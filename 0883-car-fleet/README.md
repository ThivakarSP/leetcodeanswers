...
**Runtime**: `90 ms` (Beats 27.93%)

**Memory**: `95.8 MB` (Beats 77.72%)
...
**Notes**:
The algorithm sorts cars by starting position in descending order and then greedily counts fleets by tracking the maximum arrival time seen so far; each car that arrives later than all cars ahead forms a new fleet.
